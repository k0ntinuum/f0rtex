function demo_print_symbol_key(i)
    r = rand(0:100)
    demo_alph = "O|\u2594"
    #demo_alph = "O|\u0393"
    #demo_alph = "O|X"
    h = rand(0:255)
    print(rgb(0,h,0))
    if r == 0 start_reverse_text() end
    #print(rgb(0,rand(0:255),0))
    print(demo_alph[i+1])
    if r == 0 stop_reverse_text() end
end
function demo_print_symbol_text(i)
    demo_alph = "O|\u2594"
    #demo_alph = "O|\u0393"
    #demo_alph = "O|X"
    #h = rand(0:255)
...
function demo_encode(p,f,B) 
    ...
    for i in eachindex(p)
        ...
        demo_print_symbol_text(p[i])
        cursor_to(length(f) + 5, 7 + i)
        demo_print_symbol_text(c[i])
        #sleep(0.07)
    end
    c
end
...
function demo_decode(c,f,B)
    ...
    for i in eachindex(p)
        ...
        spin(f, p[i] + 1)
        demo_print_key(f)
        #cursor_to(length(f) + 7, 7 + i)
        #demo_print_symbol(c[i])
        cursor_to(length(f) + 6 , 7 + i)
        demo_print_symbol_text(p[i])
    end
    p
end
...
function encoding_demo()
    ...
    cls()
    for i in 1:1000
        c = demo_encode(rand(0:2,25),deepcopy(f),B) 
        d = demo_decode(c,deepcopy(f),B)
    end
end
